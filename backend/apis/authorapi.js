...
authorApp.post("/article", verifytoken("AUTHOR"), async(req,res)=>{
    ...
    const articleObj = { title, category, content, author }
    //get user from decode token 
    let user=req.user
    console.log(user)
    let authorDoc=await UserModel.findById(articleObj.author)
    if(!authorDoc){
        return res.status(404).json({message:"Author Not Found"})
    }
    //check if emails are same
     if(authorDoc.email!== user.email){
         return res.status(403).json({message:"you are unauthorized"})
    }
    //check role
     if(authorDoc.role !== "AUTHOR"){
